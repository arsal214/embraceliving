...
function getCameraDevices() {
    ...
    window.agoraClient.getCameras(function(cameras) {
        ...
        // first remove all cemera options 
        jQuery('#camera-list').empty();
        cameras.forEach(function(camera, i) {
            ...
            jQuery('#camera-list').append('<option value="' + optionId + '">' + name + '</option>');
        });
...
function getMicDevices() {
    ...
    window.agoraClient.getRecordingDevices(function(mics) {
        ...
        // first remove all mic options 
        jQuery('#mic-list').empty();
        mics.forEach(function(mic, i) {
            ...
            jQuery('#mic-list').append('<option value="' + optionId + '">' + name + '</option>');
        });
    ...
    addRemoteStreamView: function(remoteStream) {
        ...
        streamsContainer.append(
            jQuery('<div/>', { 'id': streamId + '_container', 'class': 'user remote-stream-container' }).append(
                jQuery('<div/>', { 'id': streamId + '_mute', 'class': 'mute-overlay' }).append(
                    jQuery('<i/>', { 'class': 'fas fa-volume-mute' })
                ).append('<span style="font-size:14px;display:block;">Host has muted</span>'),
        ...
        remoteStream.play('agora_remote_' + streamId, function(err) {
            if (err && err.status !== "aborted") {
                ...
                jQuery("#mic-btn").attr('id', 'play-with-audio');
                document.getElementById('play-with-audio').onclick = playWithAudio;*/
                ...
                //document.getElementById(streamId + '_container').onclick = playWithAudio; // to fix autoload screen
                /*jQuery("#mic-btn").attr('id', 'play-with-audio');
                document.getElementById('play-with-audio').onclick = playWithAudio;*/

                jQuery('#start-streaming-audio').show();

                document.getElementById('start-streaming-audio').onclick = playWithAudio;
                ...
                function playWithAudio() {
                    ...
                    jQuery('#start-streaming-audio').hide();
                    /*jQuery("#play-with-audio").attr('id', 'mic-btn');*/
                }
            }
        });
    },
