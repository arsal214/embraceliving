window.AGORA_RTM_UTILS = {
    ...
    setupRTM: function(agoraAppId, channelName) {
        ...
        window.rtmChannel.on('MemberJoined', memberId => {
            ...
            updateUsersCount()
            get_remote_user_house_name(memberId);
            agora_get_user_list();
        ...
        window.rtmChannel.on('MemberLeft', memberId => {
            updateUsersCount()
            update_remote_user_house_name(memberId);
            agora_get_user_list();
        })
    ...
    joinChannel: function(uid, next) {
        function runJoin(cb) {
            ...
            const successToken = (err, token) => {
                ...
                // if (localStorage.getItem("tab_opened") == '' || localStorage.getItem("tab_opened") == undefined) {
                window.rtmClient.login(loginData).then(() => {
                    console.log('Agora RTM client login success');
                    jQuery('#settings-btn').prop('disabled', true);
                    //jQuery("#settings-btn #vid_im").attr("src", window.agora_base_url + 'img/settings-disabled.png');
                    jQuery("#settings-btn #vid_im").css('opacity', '.50');
                    jQuery("#video-settings .heygo_control_txt").css('opacity', '.50');

                    window.rtmChannel.join().then(() => {
                        cb && cb();
                        window.dispatchEvent(new CustomEvent('agora.rtm_init'));
                        localStorage.setItem("tab_opened", 1);
                    }).catch(err => {
    ...
    sendChannelMessage: function(msg, cb) {
        if (window.AGORA_RTM_UTILS.connectionState === AgoraRTM.ConnectionState.CONNECTED) {
            window.rtmChannel.sendMessage(msg, { enableHistoricalMessaging: true }).then(() => {
...
function get_remote_user_house_name(id) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        data: {
            'action': 'agora_get_participant_home_name',
            'id': id,
            'channel_id': window.channelId
        },
        success: function(result) {
            console.log("Agora MMT Stream started.")
        }
    });
}

function update_remote_user_house_name(id) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        data: {
            'action': 'agora_update_participant_home_name',
            'id': id,
            'channel_id': window.channelId
        },
        success: function(result) {
...
function delete_remote_user_house_name() {
    ...
    $.ajax({
        ...
        dataType: 'json',
        data: {
            'action': 'agora_delete_participant_home_name',
            'channel_id': window.channelId
        },
        success: function(result) {
            console.log("Agora MMT Stream started.")
...
function agora_get_user_list() {
    ...
    $.ajax({
        ...
        dataType: 'json',
        data: {
            'action': 'agora_get_users_home_list',
            'channel_id': window.channelId
        },
        success: function(result) {
            if (result.success == true) {
                if (result.data.length > 0) {
                    jQuery('#agora_participant_homes').html(result.data);
                }
            }
        }
    });
